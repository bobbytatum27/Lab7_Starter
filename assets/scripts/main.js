...
const router = new Router(function () {
   ...
   */
   let recipe_cards= document.querySelector('.section--recipe-cards');
   recipe_cards.classList.add('shown');
   let recipe_expand = document.querySelector('.section--recipe-expand');
   recipe_expand.classList.remove('shown');
});
...
function createRecipeCards() {
  ...
  for (const url in recipeData) {
    // create the html element + populate w/ fetched data
    const recipeCard = document.createElement('recipe-card');
    recipeCard.data = recipeData[url];

    // bind the recipeCard's page name with a function that changes the html to show the expanded view
    const page = recipeData[url]['page-name'];
    router.addPage(page, function() {
      document.querySelector('.section--recipe-cards').classList.remove('shown');
      document.querySelector('.section--recipe-expand').classList.add('shown');
      document.querySelector('recipe-expand').data = recipeData[url];
    });
    bindRecipeCard(recipeCard, page); // adds event listener to navigate to page for recipe card
